...
- (id)initWithFTSConnection:(YapCollectionsDatabaseFullTextSearchConnection *)inFTSConnection
...
- (BOOL)populate
{
	...
	__unsafe_unretained YapCollectionsDatabaseFullTextSearch *fts = ftsConnection->fts;
	
	BOOL needsObject = fts->blockType == YapCollectionsDatabaseFullTextSearchBlockTypeWithObject ||
	                   fts->blockType == YapCollectionsDatabaseFullTextSearchBlockTypeWithRow;
	
	BOOL needsMetadata = fts->blockType == YapCollectionsDatabaseFullTextSearchBlockTypeWithMetadata ||
	                     fts->blockType == YapCollectionsDatabaseFullTextSearchBlockTypeWithRow;
	
	if (needsObject && needsMetadata)
	{
		__unsafe_unretained YapCollectionsDatabaseFullTextSearchWithRowBlock block =
		    (YapCollectionsDatabaseFullTextSearchWithRowBlock)fts->block;
	...
	{
		__unsafe_unretained YapCollectionsDatabaseFullTextSearchWithObjectBlock block =
		    (YapCollectionsDatabaseFullTextSearchWithObjectBlock)fts->block;
	...
	{
		__unsafe_unretained YapCollectionsDatabaseFullTextSearchWithMetadataBlock block =
		    (YapCollectionsDatabaseFullTextSearchWithMetadataBlock)fts->block;
	...
	{
		__unsafe_unretained YapCollectionsDatabaseFullTextSearchWithKeyBlock block =
		    (YapCollectionsDatabaseFullTextSearchWithKeyBlock)fts->block;
                     ...
                     rowid:(int64_t)rowid
{
	...
	if (fts->blockType == YapCollectionsDatabaseFullTextSearchBlockTypeWithKey)
	{
		__unsafe_unretained YapCollectionsDatabaseFullTextSearchWithKeyBlock block =
		    (YapCollectionsDatabaseFullTextSearchWithKeyBlock)fts->block;
	...
	else if (fts->blockType == YapCollectionsDatabaseFullTextSearchBlockTypeWithObject)
	{
		__unsafe_unretained YapCollectionsDatabaseFullTextSearchWithObjectBlock block =
		    (YapCollectionsDatabaseFullTextSearchWithObjectBlock)fts->block;
	...
	else if (fts->blockType == YapCollectionsDatabaseFullTextSearchBlockTypeWithMetadata)
	{
		__unsafe_unretained YapCollectionsDatabaseFullTextSearchWithMetadataBlock block =
		    (YapCollectionsDatabaseFullTextSearchWithMetadataBlock)fts->block;
                   ...
                   withRowid:(int64_t)rowid
{
	...
	if (fts->blockType == YapCollectionsDatabaseFullTextSearchBlockTypeWithKey ||
	    fts->blockType == YapCollectionsDatabaseFullTextSearchBlockTypeWithObject)
	{
	...
	else
	{
		...
		if (fts->blockType == YapCollectionsDatabaseFullTextSearchBlockTypeWithMetadata)
		{
			__unsafe_unretained YapCollectionsDatabaseFullTextSearchWithMetadataBlock block =
		        (YapCollectionsDatabaseFullTextSearchWithMetadataBlock)fts->block;
