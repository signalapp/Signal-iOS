...
- (BOOL)populate
{
	...
	if (secondaryIndex->blockType == YapDatabaseSecondaryIndexBlockTypeWithKey)
	{
		...
		[databaseTransaction _enumerateKeysUsingBlock:^(int64_t rowid, NSString *key, BOOL *stop) {
			...
			block(secondaryIndexConnection->blockDict, key);
	...
	else if (secondaryIndex->blockType == YapDatabaseSecondaryIndexBlockTypeWithObject)
	{
		...
		[databaseTransaction _enumerateKeysAndObjectsUsingBlock:
			...
			block(secondaryIndexConnection->blockDict, key, object);
	...
	else if (secondaryIndex->blockType == YapDatabaseSecondaryIndexBlockTypeWithMetadata)
	{
		...
		[databaseTransaction _enumerateKeysAndMetadataUsingBlock:
			...
			block(secondaryIndexConnection->blockDict, key, metadata);
	...
	else // if (secondaryIndex->blockType == YapDatabaseSecondaryIndexBlockTypeWithRow)
	{
		...
		[databaseTransaction _enumerateRowsUsingBlock:
			...
			block(secondaryIndexConnection->blockDict, key, object, metadata);
