...
- (BOOL)getKey:(NSString **)keyPtr forRowid:(int64_t)rowid
{
	...
	// SELECT "key" FROM "database2" WHERE "rowid" = ?;
...
- (BOOL)getKey:(NSString **)keyPtr object:(id *)objectPtr forRowid:(int64_t)rowid
{
	...
	// SELECT "key", "data" FROM "database2" WHERE "rowid" = ?;
	...
	if (status == SQLITE_ROW)
	{
		...
		if (object == nil)
		{
			...
			object = connection->database->objectDeserializer(key, oData);
	...
	{
		YDBLogError(@"Error executing 'getKeyDataForRowidStatement': %d %s, key(%@)",
		            status, sqlite3_errmsg(connection->db), key);
	}
...
- (BOOL)getKey:(NSString **)keyPtr metadata:(id *)metadataPtr forRowid:(int64_t)rowid
{
	...
	// SELECT "key", "metadata" FROM "database2" WHERE "rowid" = ?;
	...
	if (status == SQLITE_ROW)
	{
		...
		else
		{
			...
			if (blobSize > 0)
			{
				...
				metadata = connection->database->metadataDeserializer(key, mData);
			}
	...
	{
		YDBLogError(@"Error executing 'getKeyMetadataForRowidStatement': %d %s, key(%@)",
		            status, sqlite3_errmsg(connection->db), key);
	}
...
- (BOOL)getKey:(NSString **)keyPtr object:(id *)objectPtr metadata:(id *)metadataPtr forRowid:(int64_t)rowid
{
	...
	// SELECT "key", "data", "metadata" FROM "database2" WHERE "rowid" = ?;
...
- (NSData *)primitiveDataForKey:(NSString *)key
{
	...
	// SELECT "data" FROM "database2" WHERE "key" = ? ;
...
- (NSData *)primitiveMetadataForKey:(NSString *)key
{
	...
	// SELECT "metadata" FROM "database2" WHERE "key" = ? ;
...
- (BOOL)getPrimitiveData:(NSData **)dataPtr primitiveMetadata:(NSData **)metadataPtr forKey:(NSString *)key
{
	...
	// SELECT "data", "metadata" FROM "database2" WHERE "key" = ? ;
...
- (id)metadataForKey:(NSString *)key
{
	...
	{
		if (metadataData)
			metadata = connection->database->metadataDeserializer(key, metadataData);
             ...
             unorderedUsingBlock:(void (^)(NSUInteger keyIndex, id metadata, BOOL *stop))block
{
	...
	do
	{
		...
		// SELECT "key", "metadata" FROM "database2" WHERE key IN (?, ?, ...);
		
		NSUInteger capacity = 80 + (numHostParams * 3);
		NSMutableString *query = [NSMutableString stringWithCapacity:capacity];
		
		[query appendString:@"SELECT \"key\", \"metadata\" FROM \"database2\" WHERE \"key\" IN ("];
		...
		if (status == SQLITE_ROW)
		{
			...
			do
			{
				...
				id metadata = metadataData ? connection->database->metadataDeserializer(key, metadataData) : nil;
            ...
            unorderedUsingBlock:(void (^)(NSUInteger keyIndex, id object, BOOL *stop))block
{
	...
	do
	{
		...
		// Create the SQL query:
		// SELECT "key", "data" FROM "database2" WHERE key IN (?, ?, ...);
		
		NSUInteger capacity = 80 + (numHostParams * 3);
		NSMutableString *query = [NSMutableString stringWithCapacity:capacity];
		
		[query appendString:@"SELECT \"key\", \"data\" FROM \"database2\" WHERE \"key\" IN ("];
		...
		if (status == SQLITE_ROW)
		{
			...
			do
			{
				...
				id object = connection->database->objectDeserializer(key, objectData);
         ...
         unorderedUsingBlock:(void (^)(NSUInteger keyIndex, id object, id metadata, BOOL *stop))block
{
	...
	do
	{
		...
		// Create the SQL query:
		// SELECT "key", "data", "metadata" FROM "database2" WHERE key IN (?, ?, ...);
		
		NSUInteger capacity = 80 + (numHostParams * 3);
		NSMutableString *query = [NSMutableString stringWithCapacity:capacity];
		
		[query appendString:@"SELECT \"key\", \"data\", \"metadata\" FROM \"database2\" WHERE \"key\" IN ("];
                                 ...
                                 withFilter:(BOOL (^)(int64_t rowid, NSString *key))filter
{
	...
	// SELECT "rowid", "key", "metadata" FROM "database2";
	//
	// Performance tuning:
	// Use dataWithBytesNoCopy to avoid an extra allocation and memcpy.
	//
                      ...
                      withFilter:(BOOL (^)(int64_t rowid, NSString *key))filter
{
	...
	// SELECT "rowid", "key", "data", "metadata" FROM "database2";
	//
	// Performance tuning:
	// Use dataWithBytesNoCopy to avoid an extra allocation and memcpy.
	//
...
- (void)setObject:(id)object forKey:(NSString *)key withMetadata:(id)metadata
{
	...
	if (found) // update existing row
	{
		...
		__attribute__((objc_precise_lifetime)) NSData *rawData = connection->database->objectSerializer(key, object);
		...
		__attribute__((objc_precise_lifetime)) NSData *rawMeta =
		    metadata ? connection->database->metadataSerializer(key, metadata) : nil;
...
- (void)removeObjectsForKeys:(NSArray *)keys
{
	...
	do
	{
		...
		if (YES)
		{
			// SELECT "key", "rowid" FROM "database2" WHERE "key" in (?, ?, ...);
			
			NSUInteger capacity = 60 + (numKeyParams * 3);
			NSMutableString *query = [NSMutableString stringWithCapacity:capacity];
			
			[query appendString:@"SELECT \"key\", \"rowid\" FROM \"database2\" WHERE \"key\" IN ("];
