...
+ (int16_t)congruentDifferenceMod2ToThe16From:(uint16_t)s1
                                           to:(uint16_t)s2 {
    int32_t d = (int32_t)(s2 - s1);
    if (d > INT16_MAX) d -= 1 << 16;
    return (int16_t)d;
}
...
+ (NSUInteger)largestIntegerThatIsAtMost:(NSUInteger)value
                        andIsAMultipleOf:(NSUInteger)multiple {
    require(multiple != 0);
...
+ (NSUInteger)smallestIntegerThatIsAtLeast:(NSUInteger)value
                          andIsAMultipleOf:(NSUInteger)multiple {
    require(multiple != 0);
    NSUInteger d = value / multiple;
    d *= multiple;
...
+ (uint8_t)uint8FromLowUInt4:(uint8_t)low4UInt4
                andHighUInt4:(uint8_t)highUInt4 {
    require(low4UInt4 < 0x10);
    require(highUInt4 < 0x10);
