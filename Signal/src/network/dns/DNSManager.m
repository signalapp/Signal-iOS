...
@property (nonatomic) CFStreamError error;
...
void handleDnsCompleted(CFHostRef hostRef, CFHostInfoType typeInfo, const CFStreamError* error, void* info) {
    ...
    @try {
        ...
        NSArray* ips = [addressDatas map:^(id addressData) {
            ...
            return [[[IPEndPoint alloc] initFromSockaddrData:addressData] address];
        }];
...
+ (TOCFuture*)asyncQueryAddressesForDomainName:(NSString*)domainName
    ...
    DNSManager* d = [[DNSManager alloc] init];
    ...
    CFStreamError error;
    Boolean startedSuccess = CFHostStartInfoResolution(hostRef, kCFHostAddresses, &error);
    d.error = error;
    CFRelease(hostRef);
    if (!startedSuccess) {
