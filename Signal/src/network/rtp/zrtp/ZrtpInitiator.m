...
-(HandshakePacket*) handlePacket:(HandshakePacket*)packet {
    @try {
        if      (packetExpectation == EXPECTING_NOTHING)     return nil;
        else if (packetExpectation == EXPECTING_HELLO)       return [self handleHello:packet];
        else if (packetExpectation == EXPECTING_HELLO_ACK)   return [self handleHelloAck:packet];
        else if (packetExpectation == EXPECTING_DH)          return [self handleDH:packet];
        else if (packetExpectation == EXPECTING_CONFIRM)     return [self handleConfirmOne:packet];
        else if (packetExpectation == EXPECTING_CONFIRM_ACK) return [self handleConfirmAck:packet];
        else return nil;
...
-(HandshakePacket*) handleConfirmOne:(HandshakePacket*) packet {
    ConfirmPacket* confirmPacket = [packet parsedAsConfirm1AuthenticatedWithMacKey:[masterSecret responderMacKey] andCipherKey:[masterSecret responderZrtpKey]];
