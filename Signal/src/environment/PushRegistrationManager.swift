...
@objc class PushRegistrationManager: NSObject, PKPushRegistryDelegate {
    ...
    // Notification registration confirmation is confirmed via AppDelegate
    // Before this occurs, it is not safe to assume push token requests will be acknowledged -
    // It's possible they will neither succeed nor fail, they may not be acknowledged at all.
    @objc
    ...
    private func registerUserNotificationSettings() -> Promise<Void> {
        ...
        guard fulfillRegisterUserNotificationSettingsPromise == nil else {
            Logger.info("\(TAG) previously registered user notification settings")
        ...
        Logger.info("\(TAG) validating user notification settings")
