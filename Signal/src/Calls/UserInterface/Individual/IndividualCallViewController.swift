...
class IndividualCallViewController: OWSViewController, CallObserver, CallAudioServiceDelegate {
    ...
    override func observeValue(forKeyPath: String?, of ofObject: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        CenterStageUtil.handleObservedValue(forKeyPath: forKeyPath, change: change, centerStageEnabledHandler: { [weak self] enabled in
            self?.centerStageButton.isSelected = enabled
        })
    }
