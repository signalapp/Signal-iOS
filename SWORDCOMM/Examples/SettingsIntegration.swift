...
//  Example: How to integrate SWORDCOMM settings into Signal's AppSettingsViewController
//
//  This file shows concrete examples of adding SWORDCOMM to Signal settings
//
...
Add SWORDCOMM section to your settings:
...
class AppSettingsViewController: OWSTableViewController2 {
    ...
    override func updateTableContents(_ contents: OWSTableContents) {
        ...
        // │ Add SWORDCOMM section to settings     │
        // └──────────────────────────────────┘
...
// MARK: - Example 2: Minimal SWORDCOMM Section (1 Line)
...
// MARK: - Example 3: SWORDCOMM Section with Status Indicators
...
extension AppSettingsViewController {
    ...
    func emmaSettingsSectionWithStatus() -> OWSTableSection {
        ...
        section.headerTitle = "SWORDCOMM Security & Translation"

        // Get current status
        let securityEnabled = UserDefaults.standard.bool(forKey: "SWORDCOMM.SecurityMonitoring")
        ...
        // Main SWORDCOMM settings item with status
        ...
        section.footerTitle = "SWORDCOMM provides military-grade post-quantum cryptography and on-device translation."
...
extension AppSettingsViewController {
    ...
    func emmaSettingsSectionInline() -> OWSTableSection {
        ...
        section.add(.switch(
            ...
            isOn: {
                UserDefaults.standard.bool(forKey: "SWORDCOMM.SecurityMonitoring")
            },
            target: self,
        ...
        section.add(.switch(
            ...
            isOn: {
                UserDefaults.standard.bool(forKey: "SWORDCOMM.AutoTranslate")
            },
            target: self,
        ...
        // Advanced settings
        section.add(.disclosureItem(
            withText: "Advanced SWORDCOMM Settings",
    ...
    private func didToggleSecurityMonitoring(_ sender: UISwitch) {
        UserDefaults.standard.set(sender.isOn, forKey: "SWORDCOMM.SecurityMonitoring")

        // Notify SWORDCOMM of settings change
        ...
        if sender.isOn {
            ...
            Logger.info("[SWORDCOMM] Security monitoring enabled")
        } else {
            SecurityManager.shared.stopMonitoring()
            Logger.info("[SWORDCOMM] Security monitoring disabled")
        }
    }

    @objc
    private func didToggleAutoTranslation(_ sender: UISwitch) {
        UserDefaults.standard.set(sender.isOn, forKey: "SWORDCOMM.AutoTranslate")
        ...
        if sender.isOn {
            Logger.info("[SWORDCOMM] Auto-translation enabled (on-device primary, network fallback)")
        } else {
...
extension AppSettingsViewController {
    ...
    private func downloadTranslationModel() {
        ...
        alert.addAction(UIAlertAction(title: "Download", style: .default) { _ in
            // Start download
            Logger.info("[SWORDCOMM] Starting translation model download")
...
struct LanguagePreferencesView: View {
    @AppStorage("SWORDCOMM.SourceLanguage") private var sourceLanguage = "da"
    @AppStorage("SWORDCOMM.TargetLanguage") private var targetLanguage = "en"
...
To integrate SWORDCOMM settings into Signal:
...
☐ 5. Test settings:
     ...
     - Verify SWORDCOMM section appears
     - Tap SWORDCOMM → should open full settings view
...
1. SWORDCOMM section doesn't appear in settings
   Solution: Verify iOS 15.0+ check and extension file is in target

2. Settings crash when tapped
   Solution: Ensure SWORDCOMMSettingsView is imported and compiled
...
4. Translation always uses network
   ...
   - Named correctly (SWORDCOMMTranslation_da_en.mlmodel)

5. On-device translation not available
   Solution:
   - Check model file is in app bundle
   - Run: ./SWORDCOMM/Scripts/convert_translation_model.py --quantize
