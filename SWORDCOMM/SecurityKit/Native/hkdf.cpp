//
//  hkdf.cpp
//  SWORDCOMM SecurityKit - HKDF-SHA256 Implementation
//
...
std::vector<uint8_t> HKDF::extract(
    ...
    SWORDCOMM_LOG_DEBUG("HKDF-Extract: IKM=%zu bytes, salt=%zu bytes -> PRK=32 bytes",
                   ikm.size(), salt.size());
...
std::vector<uint8_t> HKDF::expand(
    ...
    SWORDCOMM_LOG_DEBUG("HKDF-Expand: PRK=32 bytes, info=%zu bytes, iterations=%zu -> OKM=%zu bytes",
                   info.size(), n, length);
...
std::vector<uint8_t> HKDF::derive_key(
    ...
    SWORDCOMM_LOG_INFO("HKDF: Derived %zu bytes from %zu-byte IKM", length, ikm.size());
...
std::vector<std::vector<uint8_t>> HKDF::derive_keys(
    ...
    SWORDCOMM_LOG_INFO("HKDF: Derived %zu keys of %zu bytes each", key_count, key_length);
