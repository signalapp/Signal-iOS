...
void MemoryScrambler::fill_available_ram(int fill_percent) {
    ...
    SWORDCOMM_LOG_INFO("Filling %zu MB of RAM (%d%% of available)",
    ...
    try {
        ...
        SWORDCOMM_LOG_INFO("Successfully allocated %zu MB", allocated / (1024 * 1024));
        ...
        SWORDCOMM_LOG_ERROR("Memory allocation failed: %s", e.what());
    }
...
void MemoryScrambler::create_decoy_patterns(size_t size_mb) {
    ...
    try {
        ...
        SWORDCOMM_LOG_INFO("Created %zu MB of decoy patterns", size_mb);

    } catch (const std::bad_alloc& e) {
        SWORDCOMM_LOG_ERROR("Decoy pattern allocation failed: %s", e.what());
    }
}
...
size_t MemoryScrambler::get_available_memory() {
    ...
    if (host_statistics(host_port, HOST_VM_INFO,
        ...
        SWORDCOMM_LOG_ERROR("Failed to get VM statistics");
        return 0;
    }
