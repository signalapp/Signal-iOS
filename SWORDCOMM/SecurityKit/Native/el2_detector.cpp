...
bool EL2Detector::initialize() {
    ...
    if (!perf_counters_.initialize()) {
        SWORDCOMM_LOG_ERROR("Failed to initialize performance counters");
        return false;
    }
    ...
    SWORDCOMM_LOG_INFO("EL2 Detector initialized successfully");
    return true;
}
...
ThreatAnalysis EL2Detector::analyze_threat() {
    ...
    if (!initialized_) {
        SWORDCOMM_LOG_ERROR("EL2 Detector not initialized");
    ...
    SWORDCOMM_LOG_DEBUG("Threat analysis: level=%.2f, hypervisor=%.2f, timing=%d, cache=%d",
...
void EL2Detector::establish_baseline() {
    ...
    SWORDCOMM_LOG_INFO("Baseline established: cache_lat=%.2f, inst_lat=%.2f, cpi=%.4f, miss_rate=%.4f",
