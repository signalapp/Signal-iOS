               ...
               fromViewController:(UIViewController *)fromViewController
{
    ...
    if (@available(iOS 12, *)) {
        ...
        OWSFailNoProdLog(@"Make sure the progress view still looks good, and increment the version canary.");
    }
#endif
                   ...
                   fromViewController:(UIViewController *)fromViewController
{
    ...
    if ([error.domain isEqual:OWSSignalServiceKitErrorDomain] && error.code == OWSErrorCodeUntrustedIdentity) {
        ...
        if (untrustedRecipientId.length > 0) {
            ...
            OWSFailNoProdLog(@"%@ Untrusted recipient error is missing recipient id.", self.logTag);
        }
    ...
    [OWSPrimaryStorage.sharedManager.newDatabaseConnection asyncReadWriteWithBlock:^(
        ...
        switch (verificationState) {
            case OWSVerificationStateVerified: {
                OWSFailNoProdLog(@"%@ Shouldn't need to confirm identity if it was already verified", self.logTag);
                break;
            }
...
- (void)attachmentUploadProgress:(NSNotification *)notification
{
    ...
    if ([attachmentRecordId isEqual:attachmentID]) {
        if (!isnan(progress)) {
            ...
            OWSFailNoProdLog(@"%@ Invalid attachment progress.", self.logTag);
        }
    }
}
