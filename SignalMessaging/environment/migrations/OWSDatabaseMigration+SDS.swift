...
public extension OWSDatabaseMigration {
    ...
    func anyUpsert(transaction: SDSAnyWriteTransaction) {
        ...
        if let uniqueId = uniqueId {
            if OWSDatabaseMigration.anyFetch(uniqueId: uniqueId, transaction: transaction) != nil {
                isInserting = false
            } else {
                isInserting = true
            }
...
public extension OWSDatabaseMigration {
    ...
    class func anyRemoveAllWithoutInstantation(transaction: SDSAnyWriteTransaction) {
        switch transaction.writeTransaction {
        case .yapWrite(let ydbTransaction):
            ydbTransaction.removeAllObjects(inCollection: OWSDatabaseMigration.collection())
        case .grdbWrite(let grdbTransaction):
            do {
                try DatabaseMigrationRecord.deleteAll(grdbTransaction.database)
            } catch {
