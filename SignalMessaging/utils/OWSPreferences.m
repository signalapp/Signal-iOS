...
- (BOOL)isSystemCallLogEnabled
{
    if (@available(iOS 11, *)) {
        ...
        OWSFailNoProdLog(@"%@ Call Logging can only be configured on iOS11+", self.logTag);
        return NO;
    }
...
{
    if (@available(iOS 11, *)) {
        OWSFailNoProdLog(@"%@ CallKit is always enabled for iOS11+", self.logTag);
        return YES;
    }
...
{
    if (@available(iOS 11, *)) {
        OWSFailNoProdLog(@"%@ CallKit privacy is irrelevant for iOS11+", self.logTag);
        return NO;
    }
