...
//
//  This file demonstrates EMMA's translation architecture:
...
@available(iOS 15.0, *)
class EMMATranslationPriorityManager {

    static let shared = EMMATranslationPriorityManager()
    ...
    func translateText(
        ...
        if onDeviceEngine.isModelLoaded() {
            Logger.info("[EMMA Translation] Attempting on-device translation")

            if let result = tryOnDeviceTranslation(text, from: sourceLanguage, to: targetLanguage) {
                Logger.info("[EMMA Translation] ✓ On-device translation succeeded")
                translationStats.onDeviceTranslations += 1
                completion(result)
                return
            }

            Logger.warn("[EMMA Translation] On-device translation failed, trying fallback")
        } else {
            Logger.warn("[EMMA Translation] CoreML model not loaded, using network fallback")
        }
        ...
        if UserDefaults.standard.bool(forKey: "EMMA.NetworkFallbackEnabled") {
            Logger.info("[EMMA Translation] Using network fallback")
            ...
            Logger.warn("[EMMA Translation] Network fallback disabled, translation failed")
            completion(nil)
        }
    }
    ...
    private func tryOnDeviceTranslation(
        ...
        Logger.info("[EMMA Translation] On-device translation: \(elapsed)ms")
    ...
    private func tryNetworkTranslation(
        ...
        guard isNetworkAvailable() else {
            Logger.warn("[EMMA Translation] Network unavailable, translation failed")
            completion(nil)
            return
        }

        Logger.warn("[EMMA Translation] ⚠️ Using network fallback (on-device preferred)")
    ...
    private func performNetworkTranslation(
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            guard let data = data,
                ...
                Logger.error("[EMMA Translation] Network translation failed")
...
class MessageCellWithTranslation: UITableViewCell {
    ...
    private let translationView = EMMAMessageTranslationView()
    ...
    func configure(with message: String, shouldTranslate: Bool) {
        ...
        if shouldTranslate {
            ...
            let manager = EMMATranslationPriorityManager.shared
            ...
            manager.translateText(
                ...
                if result.usedNetwork {
                    Logger.debug("[EMMA] Translation used network fallback")
                } else {
                    Logger.debug("[EMMA] Translation used on-device CoreML")
                }
            }
        } else {
...
struct TranslationSettingsWithStats: View {
    @AppStorage("EMMA.AutoTranslate") private var autoTranslate = false
    @AppStorage("EMMA.NetworkFallbackEnabled") private var networkFallback = true
    @State private var translationStats: EMMATranslationPriorityManager.TranslationStats?
            ...
            Section(header: Text("Translation Method")) {
                ...
                Toggle("Enable Network Fallback", isOn: $networkFallback)
                    .onChange(of: networkFallback) { value in
                        if value {
                            Logger.info("[EMMA] Network fallback enabled")
                        } else {
                            Logger.warn("[EMMA] Network fallback disabled - translations may fail without on-device model")
                        }
                    }
    ...
    private func loadStatistics() {
        let stats = EMMATranslationPriorityManager.shared.getStatistics()
        translationStats = stats

        // Log to console
        EMMATranslationPriorityManager.shared.logStatistics()
    }

    private func downloadModel() {
        // Trigger model download
        Logger.info("[EMMA] Starting on-device model download")
    ...
    private static func attemptTranslation(
        ...
        manager.translateText(text, from: sourceLanguage, to: targetLanguage) { result in
            if let result = result {
                ...
                // Retry
                Logger.warn("[EMMA] Translation attempt \(attempt) failed, retrying...")
                attemptTranslation(
                    text: text,
                    from: sourceLanguage,
                    to: targetLanguage,
                ...
                Logger.error("[EMMA] Translation failed after \(attempt) attempts")
                completion(nil)
            }
        }
    }
