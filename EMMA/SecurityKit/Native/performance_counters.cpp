...
bool PerformanceCounters::initialize() {
    ...
    if (mach_timebase_info(&timebase_) != KERN_SUCCESS) {
        EMMA_LOG_ERROR("Failed to get mach timebase info");
        return false;
    }
    ...
    if (!read_task_info(baseline_vm_info_)) {
        EMMA_LOG_ERROR("Failed to read initial task info");
        return false;
    }

    initialized_ = true;
    EMMA_LOG_INFO("Performance counters initialized (iOS mach API mode)");
    return true;
}

bool PerformanceCounters::read_counters(PerfCounterData& data) {
    if (!initialized_) {
        EMMA_LOG_ERROR("Performance counters not initialized");
        return false;
    }
    ...
    if (!read_task_info(vm_info)) {
        EMMA_LOG_ERROR("Failed to read task info");
        return false;
    }
    ...
    if (!read_thread_basic_info(thread_info)) {
        EMMA_LOG_ERROR("Failed to read thread info");
        return false;
    }
...
bool PerformanceCounters::read_task_info(task_vm_info_data_t& info) {
    ...
    if (kr != KERN_SUCCESS) {
        EMMA_LOG_ERROR("task_info failed: %d", kr);
        return false;
    }
...
bool PerformanceCounters::read_thread_basic_info(thread_basic_info_data_t& info) {
    ...
    if (kr != KERN_SUCCESS) {
        EMMA_LOG_ERROR("thread_info failed: %d", kr);
        return false;
    }
