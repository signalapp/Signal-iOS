...
#if LIBOQS_AVAILABLE
    EMMA_LOG_INFO("liboqs initialization - version: %s", OQS_VERSION);

    // Verify ML-KEM-1024 is available
    if (!OQS_KEM_alg_is_enabled(OQS_KEM_alg_ml_kem_1024)) {
        EMMA_LOG_ERROR("ML-KEM-1024 not enabled in liboqs build");
        return false;
    }

    // Verify ML-DSA-87 is available
    if (!OQS_SIG_alg_is_enabled(OQS_SIG_alg_ml_dsa_87)) {
        EMMA_LOG_ERROR("ML-DSA-87 not enabled in liboqs build");
        return false;
    }

    EMMA_LOG_INFO("liboqs initialized successfully - ML-KEM-1024 and ML-DSA-87 enabled");
    return true;
#else
    EMMA_LOG_WARN("liboqs NOT COMPILED - using stub implementations");
    EMMA_LOG_WARN("To enable production crypto, build with HAVE_LIBOQS=1");
...
int liboqs_ml_kem_1024_keypair(uint8_t *public_key, uint8_t *secret_key) {
    if (!public_key || !secret_key) {
        EMMA_LOG_ERROR("NULL pointer passed to ml_kem_1024_keypair");
        return -1;
    }
    ...
    if (!kem) {
        EMMA_LOG_ERROR("Failed to create ML-KEM-1024 context");
        return -1;
    }

    // Verify sizes match our constants
    if (kem->length_public_key != LIBOQS_ML_KEM_1024_PUBLIC_KEY_BYTES) {
        EMMA_LOG_ERROR("ML-KEM-1024 public key size mismatch: %zu != %d",
    ...
    if (kem->length_secret_key != LIBOQS_ML_KEM_1024_SECRET_KEY_BYTES) {
        EMMA_LOG_ERROR("ML-KEM-1024 secret key size mismatch: %zu != %d",
    ...
    if (status != OQS_SUCCESS) {
        EMMA_LOG_ERROR("ML-KEM-1024 keypair generation failed");
        return -1;
    }

    EMMA_LOG_INFO("ML-KEM-1024 keypair generated successfully");
    return 0;
#else
    // STUB IMPLEMENTATION - DO NOT USE IN PRODUCTION
    EMMA_LOG_WARN("STUB: ML-KEM-1024 keypair generation (NOT SECURE)");
    ...
    if (!secure_random_bytes(public_key, LIBOQS_ML_KEM_1024_PUBLIC_KEY_BYTES) ||
        ...
        EMMA_LOG_ERROR("Failed to generate random stub keys");
        return -1;
    }
...
int liboqs_ml_kem_1024_encapsulate(
    ...
    if (!ciphertext || !shared_secret || !public_key) {
        EMMA_LOG_ERROR("NULL pointer passed to ml_kem_1024_encapsulate");
        return -1;
    }
    ...
    if (status != OQS_SUCCESS) {
        EMMA_LOG_ERROR("ML-KEM-1024 encapsulation failed");
        return -1;
    }
...
#else
    // STUB IMPLEMENTATION - DO NOT USE IN PRODUCTION
    EMMA_LOG_WARN("STUB: ML-KEM-1024 encapsulation (NOT SECURE)");
    ...
    if (!secure_random_bytes(ciphertext, LIBOQS_ML_KEM_1024_CIPHERTEXT_BYTES) ||
        ...
        EMMA_LOG_ERROR("Failed to generate random stub encapsulation");
        return -1;
    }
...
int liboqs_ml_kem_1024_decapsulate(
    ...
    if (!shared_secret || !ciphertext || !secret_key) {
        EMMA_LOG_ERROR("NULL pointer passed to ml_kem_1024_decapsulate");
        return -1;
    }
    ...
    if (status != OQS_SUCCESS) {
        EMMA_LOG_ERROR("ML-KEM-1024 decapsulation failed");
        return -1;
    }
...
#else
    // STUB IMPLEMENTATION - DO NOT USE IN PRODUCTION
    EMMA_LOG_WARN("STUB: ML-KEM-1024 decapsulation (NOT SECURE)");
    ...
    if (!secure_random_bytes(shared_secret, LIBOQS_ML_KEM_1024_SHARED_SECRET_BYTES)) {
        EMMA_LOG_ERROR("Failed to generate random stub shared secret");
        return -1;
    }
...
int liboqs_ml_dsa_87_keypair(uint8_t *public_key, uint8_t *secret_key) {
    if (!public_key || !secret_key) {
        EMMA_LOG_ERROR("NULL pointer passed to ml_dsa_87_keypair");
        return -1;
    }
    ...
    if (!sig) {
        EMMA_LOG_ERROR("Failed to create ML-DSA-87 context");
        return -1;
    }

    // Verify sizes match our constants
    if (sig->length_public_key != LIBOQS_ML_DSA_87_PUBLIC_KEY_BYTES) {
        EMMA_LOG_ERROR("ML-DSA-87 public key size mismatch: %zu != %d",
    ...
    if (sig->length_secret_key != LIBOQS_ML_DSA_87_SECRET_KEY_BYTES) {
        EMMA_LOG_ERROR("ML-DSA-87 secret key size mismatch: %zu != %d",
    ...
    if (status != OQS_SUCCESS) {
        EMMA_LOG_ERROR("ML-DSA-87 keypair generation failed");
        return -1;
    }

    EMMA_LOG_INFO("ML-DSA-87 keypair generated successfully");
    return 0;
#else
    // STUB IMPLEMENTATION - DO NOT USE IN PRODUCTION
    EMMA_LOG_WARN("STUB: ML-DSA-87 keypair generation (NOT SECURE)");
...
int liboqs_ml_dsa_87_sign(
    ...
    if (!signature || !signature_len || !message || !secret_key) {
        EMMA_LOG_ERROR("NULL pointer passed to ml_dsa_87_sign");
        return -1;
    }
    ...
    if (status != OQS_SUCCESS) {
        EMMA_LOG_ERROR("ML-DSA-87 signature generation failed");
        return -1;
    }

    EMMA_LOG_DEBUG("ML-DSA-87 signature generated (%zu bytes)", *signature_len);
    return 0;
#else
    // STUB IMPLEMENTATION - DO NOT USE IN PRODUCTION
    EMMA_LOG_WARN("STUB: ML-DSA-87 signing (NOT SECURE)");

    *signature_len = LIBOQS_ML_DSA_87_SIGNATURE_BYTES;
    if (!secure_random_bytes(signature, LIBOQS_ML_DSA_87_SIGNATURE_BYTES)) {
        EMMA_LOG_ERROR("Failed to generate random stub signature");
        return -1;
    }
...
int liboqs_ml_dsa_87_verify(
    ...
    if (!message || !signature || !public_key) {
        EMMA_LOG_ERROR("NULL pointer passed to ml_dsa_87_verify");
        return -1;
    }
...
#else
    // STUB IMPLEMENTATION - DO NOT USE IN PRODUCTION
    EMMA_LOG_WARN("STUB: ML-DSA-87 verification (ALWAYS SUCCEEDS - NOT SECURE)");
