...
void CacheOperations::poison_cache(int intensity_percent) {
    ...
    EMMA_LOG_DEBUG("Cache poisoned with intensity %d%%", intensity_percent);
}
...
void CacheOperations::fill_cache_with_noise(size_t size_kb) {
    ...
    if (!emma::platform::secure_random_bytes(noise.data(), size_bytes)) {
        EMMA_LOG_ERROR("Failed to generate random noise for cache fill");
        return;
    }
    ...
    EMMA_LOG_DEBUG("Filled cache with %zu KB of noise", size_kb);
}
