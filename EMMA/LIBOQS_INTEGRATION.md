# liboqs Integration Guide for EMMA iOS
...
EMMA iOS uses **liboqs** for production-grade post-quantum cryptography. This document explains how to integrate liboqs into the iOS build.
...
**Current status**: EMMA is in stub mode by default.

### Characteristics
- ✅ Works immediately, no setup required
- ✅ Allows UI and integration testing
- ❌ **NOT SECURE** - uses random data instead of real crypto
- ❌ Cannot communicate with EMMA-Android
- ❌ Signatures always verify (false positives)

### Detection
When EMMA runs in stub mode, you'll see log messages:
```
[EMMA] liboqs NOT COMPILED - using stub implementations
[EMMA] Generated ML-KEM-1024 keypair - STUB MODE (NOT SECURE)
[EMMA] Generated ML-DSA-87 keypair - STUB MODE (NOT SECURE)
```

### To Continue in Stub Mode
No action needed. EMMA will continue to work for development and testing, but **do not use in production**.
...
echo "   Copy it to EMMA/Frameworks/"
```
...
```bash
# Copy to EMMA frameworks directory
mkdir -p EMMA/Frameworks
cp -R liboqs-build/liboqs-0.10.1/liboqs.xcframework EMMA/Frameworks/
```

### Step 3: Update Podfile

Edit `EMMA/EMMASecurityKit.podspec`:

```ruby
Pod::Spec.new do |s|
  s.name         = 'EMMASecurityKit'
  s.version      = '1.2.0'
  # ... existing config ...
...
if(LIBOQS_LIBRARY)
    ...
    target_link_libraries(EMMASecurityKit PRIVATE ${LIBOQS_LIBRARY})
else()
...
**Expected log output**:
```
[EMMA] liboqs initialization - version: 0.10.1
[EMMA] liboqs initialized successfully - ML-KEM-1024 and ML-DSA-87 enabled
[EMMA] Generated ML-KEM-1024 keypair (NIST FIPS 203) - PRODUCTION
[EMMA] Generated ML-DSA-87 keypair (NIST FIPS 204) - PRODUCTION
```
...
Create `EMMA/ThirdParty/build_liboqs.sh`:
...
Add **Run Script** phase in EMMA SecurityKit target:

```bash
# Build liboqs if not already built
if [ ! -f "${PROJECT_DIR}/EMMA/ThirdParty/liboqs/build-ios/lib/liboqs.a" ]; then
    echo "Building liboqs..."
    bash "${PROJECT_DIR}/EMMA/ThirdParty/build_liboqs.sh"
fi
```
...
**Header Search Paths**:
```
$(PROJECT_DIR)/EMMA/ThirdParty/liboqs/build-ios/include
```

**Library Search Paths**:
```
$(PROJECT_DIR)/EMMA/ThirdParty/liboqs/build-ios/lib
```
...
2. **Send public key to EMMA-Android**
...
**Fix**: Add `$(PROJECT_DIR)/EMMA/Frameworks/liboqs.xcframework/Headers` to **Header Search Paths**
...
- [ ] Cross-platform communication tested with EMMA-Android
...
- **EMMA Wrapper API**: `EMMA/SecurityKit/Native/liboqs_wrapper.h`
