...
OPUS-MT to CoreML Model Converter for EMMA Translation
...
def convert_to_coreml(model, tokenizer, model_dir, output_path, quantize=False):
    ...
    try:
        ...
        # Add metadata
        mlmodel.author = "EMMA Translation Kit"
...
def print_integration_instructions(model_path, source_lang, target_lang):
    """Print instructions for integrating the model into EMMA"""
    print()
    print("━" * 70)
    ...
    print("   - Drag EMMATranslation.mlmodel into Xcode")
    ...
    print("  - EMMA/COREML_TRANSLATION_GUIDE.md")
    print("  - EMMA/TranslationKit/EMTranslationEngine.swift")
    print()
    print("━" * 70)

def main():
    parser = argparse.ArgumentParser(
        description="Convert OPUS-MT model to CoreML format for EMMA"
    )
    parser.add_argument(
        "--source",
        default="da",
    ...
    parser.add_argument(
        "--output",
        help="Output directory (default: EMMA/TranslationKit/Models)"
    )
    ...
    if args.output:
        ...
        output_dir = project_root / "EMMA" / "TranslationKit" / "Models"
    ...
    # Convert to CoreML
    model_name = f"EMMATranslation_{args.source}_{args.target}"
    if args.quantize:
        model_name += "_int8"
    model_name += ".mlmodel"
