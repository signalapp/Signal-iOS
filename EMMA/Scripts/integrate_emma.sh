...
#
# integrate_emma.sh
# Automated EMMA Integration Script for Signal-iOS
#
# This script automates the integration of EMMA into Signal-iOS by:
# 1. Adding EMMA extension files to Xcode project
# 2. Inserting integration code into Signal source files
# 3. Updating build configuration
# 4. Running verification tests
#
# Usage: ./integrate_emma.sh [--dry-run] [--verbose]
#
...
EMMA_ROOT="$PROJECT_ROOT/EMMA"

DRY_RUN=false
VERBOSE=false
BACKUP_DIR="$PROJECT_ROOT/.emma_backup_$(date +%Y%m%d_%H%M%S)"
...
check_prerequisites() {
    ...
    # Check if EMMA directory exists
    if [ ! -d "$EMMA_ROOT" ]; then
        log_error "EMMA directory not found: $EMMA_ROOT"
        exit 1
    fi
...
# Step 1: Add EMMA extension files to Xcode project
add_emma_extensions() {
    log_info "Step 1: Adding EMMA extension files to Xcode project..."

    local extensions=(
        "Integration/SignalAppDelegate+EMMA.swift"
        "Integration/SignalSettingsViewController+EMMA.swift"
        "Integration/SignalConversationViewController+EMMA.swift"
        "Integration/SignalMessageTranslation+EMMA.swift"
    )

    for ext in "${extensions[@]}"; do
        local source="$EMMA_ROOT/$ext"
        if [ ! -f "$source" ]; then
            log_error "EMMA extension file not found: $source"
            exit 1
        fi
...
patch_app_delegate() {
    ...
    if grep -q "initializeEMMA()" "$app_delegate"; then
    ...
    if [ "$DRY_RUN" = false ]; then
        ...
        log_info "  Adding EMMA initialization..."
        # In real implementation, use sed or awk to insert code
        log_info "  Would insert: if #available(iOS 15.0, *), isEMMAEnabled { initializeEMMA() }"

        # Patch 2: Add to didBecomeActive
        log_info "  Adding didBecomeActive hook..."
        log_info "  Would insert: if #available(iOS 15.0, *), isEMMAEnabled { emmaDidBecomeActive() }"

        # Patch 3: Add to didEnterBackground
        log_info "  Adding didEnterBackground hook..."
        log_info "  Would insert: if #available(iOS 15.0, *), isEMMAEnabled { emmaDidEnterBackground() }"
...
patch_settings() {
    ...
    if [ "$DRY_RUN" = false ]; then
        ...
        log_info "  Adding EMMA settings section..."
        log_info "  Would insert: let emmaSection = emmaSettingsSection()"
        log_warning "  NOTE: Automatic patching not yet implemented"
        log_info "  Please manually add EMMA section to updateTableContents()"
    else
...
update_podfile() {
    ...
    if grep -q "EMMASecurityKit" "$podfile"; then
        log_warning "Podfile already includes EMMA pods"
        return
    fi
    ...
    if [ "$DRY_RUN" = false ]; then
        ...
        log_info "  Adding EMMA pods..."
        # Append EMMA pods after Signal target
        cat >> "$podfile" << 'EOF'
...
EOF
        log_success "EMMA pods added to Podfile"
    else
        log_info "  [DRY RUN] Would add EMMA pods to Podfile"
    fi
}
...
verify_integration() {
    ...
    # Check 1: EMMA extension files exist
    if [ -f "$EMMA_ROOT/Integration/SignalAppDelegate+EMMA.swift" ]; then
        log_success "  ✓ EMMA extension files found"
        ((checks_passed++))
    else
        log_error "  ✗ EMMA extension files missing"
    fi

    # Check 2: Podfile includes EMMA
    if grep -q "EMMASecurityKit" "$SIGNAL_ROOT/Podfile"; then
        log_success "  ✓ Podfile includes EMMA"
        ((checks_passed++))
    else
        log_warning "  ✗ Podfile does not include EMMA"
    fi

    # Check 3: Pods directory exists
    if [ -d "$SIGNAL_ROOT/Pods/EMMASecurityKit" ]; then
        log_success "  ✓ EMMA pods installed"
        ((checks_passed++))
    else
        log_warning "  ✗ EMMA pods not installed (run pod install)"
    fi
    ...
    if [ -f "$EMMA_ROOT/EMMA-Bridging-Header.h" ]; then
...
print_next_steps() {
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo -e "${GREEN}EMMA Integration Script Complete!${NC}"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
    echo "Next Steps:"
    echo ""
    ...
    echo ""
    echo "2. Add EMMA extension files to Signal target in Xcode:"
    echo "   - SignalAppDelegate+EMMA.swift"
    echo "   - SignalSettingsViewController+EMMA.swift"
    echo "   - SignalConversationViewController+EMMA.swift (optional)"
    echo "   - SignalMessageTranslation+EMMA.swift (optional)"
    echo ""
    echo "3. Manually add integration code (3-5 calls):"
    echo "   See: EMMA/PHASE4_SIGNAL_INTEGRATION.md"
    echo ""
    ...
    echo "   Look for: [EMMA] Initializing EMMA Security & Translation"
    echo ""
    ...
    echo "  - EMMA/PHASE4_SIGNAL_INTEGRATION.md"
...
main() {
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "  EMMA Signal-iOS Integration Script"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
