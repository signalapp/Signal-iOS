...
EMMA iOS uses on-device neural machine translation for Danish-English translation. This guide explains how to convert and integrate the OPUS-MT MarianMT model into CoreML format.

### Translation Architecture

```
┌──────────────────────────────────────────────────────────────┐
│                     EMMA Translation Stack                    │
├──────────────────────────────────────────────────────────────┤
...
mlmodel.author = "Helsinki-NLP / EMMA"
...
1. **Copy files to project**:
   ```bash
   cp opus-mt-da-en-int8.mlmodel EMMA/TranslationKit/Models/
   cp opus-mt-da-en_vocab.json EMMA/TranslationKit/Resources/
   ```
...
public class TranslationManager {
    ...
    public func initialize() -> Bool {
        ...
        do {
            ...
            NSLog("[EMMA] Failed to load CoreML translation model: \(error)")
            return false
        }
        ...
        guard let vocabURL = Bundle.main.url(forResource: "opus-mt-da-en_vocab", withExtension: "json"),
            ...
            NSLog("[EMMA] Failed to load vocabulary")
            return false
        }
        ...
        NSLog("[EMMA] Translation model initialized (vocab size: \(vocabulary.count))")
        return true
    }

    public func translate(_ text: String, from: String, to: String) -> TranslationResult? {
        guard let model = model else {
            NSLog("[EMMA] Translation model not loaded")
            return nil
        }
        ...
        guard let output = try? model.prediction(input_ids: inputIDsArray, attention_mask: attentionMaskArray) else {
            NSLog("[EMMA] Translation inference failed")
            return nil
        }
...
func downloadModel() async -> Bool {
    let url = URL(string: "https://emma.sword.ai/models/opus-mt-da-en-int8.mlmodel")!
...
- **EMMA Translation API**: `EMMA/TranslationKit/Swift/TranslationManager.swift`
