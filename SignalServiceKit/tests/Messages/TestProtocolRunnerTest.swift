...
class TestProtocolRunnerTest: SSKBaseTest {
    ...
    func test_roundtrip() {
        write { transaction in
            ...
            let cipherMessage = try! self.runner.encrypt(
            ...
            let decrypted = try! self.runner.decrypt(
                cipherMessage,
    ...
    func test_multiple() {
        write { transaction in
            ...
            let cipherMessage1 = try! self.runner.encrypt(
                plaintext1.data(using: .utf8)!,
                senderClient: self.aliceClient,
                recipient: self.bobClient.protocolAddress,
                context: NullContext(),
            )
            ...
            let cipherMessage2 = try! self.runner.encrypt(
                plaintext2.data(using: .utf8)!,
                senderClient: self.aliceClient,
                recipient: self.bobClient.protocolAddress,
                context: NullContext(),
            )
            ...
            // two decrypts
            let decrypted1 = try! self.runner.decrypt(
                cipherMessage1,
                recipientClient: self.bobClient,
                sender: self.aliceClient.protocolAddress,
                context: NullContext(),
            ...
            let decrypted2 = try! self.runner.decrypt(
                cipherMessage2,
                recipientClient: self.bobClient,
                sender: self.aliceClient.protocolAddress,
                context: NullContext(),
