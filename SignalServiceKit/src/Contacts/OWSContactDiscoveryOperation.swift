...
class CDSBatchOperation: OWSOperation {
    ...
    func parseAndDecrypt(response: Any?, remoteAttestation: RemoteAttestation) throws -> Data {
        ...
        let cipherText = try params.requireBase64EncodedData(key: "data")
        let initializationVector = try params.requireBase64EncodedData(key: "iv")
        let authTag = try params.requireBase64EncodedData(key: "mac")
