...
/// An Objective-C wrapper around os_unfair_lock. This is a non-FIFO, priority preserving lock. For more info, see os/lock.h
///
/// @discussion Why is this necessary? os_unfair_lock has some unexpected behavior in Swift. These problems arise from Swift's handling
/// of inout C structs. Passing the underlying struct as an inout parameter results in surprising Law of Exclusivity violations.
/// There are two ways to work around this: Manually allocate heap storage in Swift or bridge to Objective-C. I figured bridging a simple struct
...
- (BOOL)tryLock NS_SWIFT_NAME(tryLock()); // Note: NS_SWIFT_NAME is required to prevent bridging from renaming to `try()`.
