...
//     do {
//         // Required
//         let name: String = try paramParser.require(key: "name")
//         let count: UInt32 = try paramParser.require(key: "count")
//
//         // Optional
//         let last_seen: Date? = try paramParser.allow(key: "last_seen")
//
...
public class ParamParser {
    ...
    public func require<T>(key: Key) throws -> T {
        guard let value: T = try allow(key: key) else {
    ...
    public func allow<T>(key: Key) throws -> T? {
    ...
    public func require<T>(key: Key) throws -> T where T: FixedWidthInteger {
        guard let value: T = try allow(key: key) else {
            throw missing(key: key)
        }
    ...
    public func allow<T>(key: Key) throws -> T? where T: FixedWidthInteger {
        guard let someValue = dictionary[key] else {
            return nil
        }
    ...
    public func requireBase64EncodedData(key: Key) throws -> Data {
        guard let data: Data = try allowBase64EncodedData(key: key) else {
    ...
    public func allowBase64EncodedData(key: Key) throws -> Data? {
        guard let encodedData: String = try self.allow(key: key) else {
            return nil
        }
