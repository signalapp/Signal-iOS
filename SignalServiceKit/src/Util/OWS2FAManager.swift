...
extension OWS2FAManager {
    ...
    private static var isUsingRandomPinKey = "isUsingRandomPinKey"

    @objc
    public var isUsingRandomPin: Bool {
        return databaseStorage.read { transaction in
            OWS2FAManager.keyValueStore().getBool(
                OWS2FAManager.isUsingRandomPinKey,
                defaultValue: false,
                transaction: transaction
            )
        }
    ...
    @objc
    public func isUsingRandomPin(transaction: SDSAnyReadTransaction) -> Bool {
        return OWS2FAManager.keyValueStore().getBool(
            OWS2FAManager.isUsingRandomPinKey,
            defaultValue: false,
            transaction: transaction
    ...
    @objc
    public func setIsUsingRandomPin(_ value: Bool, transaction: SDSAnyWriteTransaction) {
        OWS2FAManager.keyValueStore().setBool(
            value,
            key: OWS2FAManager.isUsingRandomPinKey,
            transaction: transaction
        )
    }
    ...
    @objc
    @available(swift, obsoleted: 1.0)
    public func markEnabledWithRandomPin() -> AnyPromise {
        return AnyPromise(markEnabledWithRandomPin())
    }

    public func markEnabledWithRandomPin() -> Promise<Void> {
        let highEntropyPIN = Cryptography.generateRandomBytes(32).hexadecimalString

        return firstly {
            requestEnable2FA(withPin: highEntropyPIN, mode: .V2, rotateMasterKey: true)
        }.done(on: .global()) {
            self.databaseStorage.write { transaction in
                self.setIsUsingRandomPin(true, transaction: transaction)
            }

            firstly {
                TSAccountManager.sharedInstance().updateAccountAttributes()
            }.catch { error in
                Logger.error("Error updating account attributes: \(error)")
            }
            ...
            self.databaseStorage.write { transaction in
                OWS2FAManager.keyValueStore().removeValue(
                    forKey: OWS2FAManager.isUsingRandomPinKey,
                    transaction: transaction
                )
            }
