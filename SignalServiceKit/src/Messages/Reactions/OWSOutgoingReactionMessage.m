...
- (void)updateWithSendingError:(NSError *)error transaction:(SDSAnyWriteTransaction *)transaction
{
    ...
    if (self.sentRecipientAddresses.count > 0) {
        OWSLogError(@"Failed to send reaction to some recipients: %@", error.errorLocalizedDescription);
        return;
    }
    ...
    OWSLogError(@"Failed to send reaction to all recipients: %@", error.errorLocalizedDescription);
