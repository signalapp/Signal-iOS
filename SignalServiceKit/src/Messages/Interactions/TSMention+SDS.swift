...
public struct MentionRecord: SDSRecord {
...
public extension MentionRecord {
    ...
    init(row: Row) {
        ...
        recordType = row[1]
        uniqueId = row[2]
        uniqueMessageId = row[3]
        uniqueThreadId = row[4]
        uuidString = row[5]
...
public extension String.StringInterpolation {
    mutating func appendInterpolation(mentionColumn column: MentionRecord.CodingKeys) {
        appendLiteral(MentionRecord.columnName(column))
    }
    mutating func appendInterpolation(mentionColumnFullyQualified column: MentionRecord.CodingKeys) {
        appendLiteral(MentionRecord.columnName(column, fullyQualified: true))
    }
}
...
extension TSMention {
    ...
    class func fromRecord(_ record: MentionRecord) throws -> TSMention {
        ...
        case .mention:
            ...
            let creationTimestampInterval: Double = record.creationTimestamp
            ...
            return TSMention(grdbId: recordId,
                             uniqueId: uniqueId,
...
extension TSMention: SDSModel {
    public var serializer: SDSSerializer {
        ...
        switch self {
        default:
            return TSMentionSerializer(model: self)
        }
    }
...
extension TSMention: DeepCopyable {
    ...
    public func deepCopy() throws -> AnyObject {
        ...
        do {
            ...
            assert(type(of: modelToCopy) == TSMention.self)
...
public extension TSMention {
    ...
    func anyUpsert(transaction: SDSAnyWriteTransaction) {
        ...
        if TSMention.anyFetch(uniqueId: uniqueId, transaction: transaction) != nil {
            isInserting = false
        } else {
            isInserting = true
        }
    ...
    func anyUpdate(transaction: SDSAnyWriteTransaction, block: (TSMention) -> Void) {
...
public class TSMentionCursor: NSObject {
    ...
    init(transaction: GRDBReadTransaction, cursor: RecordCursor<MentionRecord>?) {
        self.transaction = transaction
        self.cursor = cursor
    }

    public func next() throws -> TSMention? {
        ...
        return try TSMention.fromRecord(record)
    }

    public func all() throws -> [TSMention] {
        var result = [TSMention]()
        while true {
...
@objc
public extension TSMention {
    class func grdbFetchCursor(transaction: GRDBReadTransaction) -> TSMentionCursor {
        let database = transaction.database
        do {
            let cursor = try MentionRecord.fetchCursor(database)
            return TSMentionCursor(transaction: transaction, cursor: cursor)
        } catch {
            owsFailDebug("Read failed: \(error)")
            return TSMentionCursor(transaction: transaction, cursor: nil)
        }
    }

    // Fetches a single model by "unique id".
    class func anyFetch(uniqueId: String,
                        transaction: SDSAnyReadTransaction) -> TSMention? {
    ...
    class func anyFetchAll(transaction: SDSAnyReadTransaction) -> [TSMention] {
        var result = [TSMention]()
...
public extension TSMention {
    class func grdbFetchCursor(sql: String,
                               ...
                               transaction: GRDBReadTransaction) -> TSMentionCursor {
        do {
            let sqlRequest = SQLRequest<Void>(sql: sql, arguments: arguments, cached: true)
            let cursor = try MentionRecord.fetchCursor(transaction.database, sqlRequest)
            return TSMentionCursor(transaction: transaction, cursor: cursor)
        } catch {
    ...
    class func grdbFetchOne(sql: String,
                            arguments: StatementArguments = StatementArguments(),
                            transaction: GRDBReadTransaction) -> TSMention? {
        assert(sql.count > 0)

        do {
            let sqlRequest = SQLRequest<Void>(sql: sql, arguments: arguments, cached: true)
            guard let record = try MentionRecord.fetchOne(transaction.database, sqlRequest) else {
                return nil
            }
...
class TSMentionSerializer: SDSSerializer {

    private let model: TSMention
    public required init(model: TSMention) {
        self.model = model
    }
    ...
    func asRecord() throws -> SDSRecord {
        ...
        let recordType: SDSRecordType = .mention
        let uniqueId: String = model.uniqueId

        // Properties
        let uniqueMessageId: String = model.uniqueMessageId
        let uniqueThreadId: String = model.uniqueThreadId
        let uuidString: String = model.uuidString
        let creationTimestamp: Double = archiveDate(model.creationTimestamp)

        return MentionRecord(delegate: model, id: id, recordType: recordType, uniqueId: uniqueId, uniqueMessageId: uniqueMessageId, uniqueThreadId: uniqueThreadId, uuidString: uuidString, creationTimestamp: creationTimestamp)
    }
}
...
public extension TSMention {
    ...
    func deepCopyUsingRecord() throws -> TSMention {
        guard let record = try asRecord() as? MentionRecord else {
