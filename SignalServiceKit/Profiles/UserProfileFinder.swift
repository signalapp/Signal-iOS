...
public class UserProfileFinder: NSObject {
    ...
    func userProfiles(for addresses: [OWSUserProfile.Address], tx: DBReadTransaction) -> [OWSUserProfile?] {
    ...
    func fetchUserProfiles(serviceId: ServiceId, tx: DBReadTransaction) -> [OWSUserProfile] {
    ...
    func fetchUserProfiles(phoneNumber: String, tx: DBReadTransaction) -> [OWSUserProfile] {
        let userProfiles = userProfilesWhere(
    ...
    func fetchAcisWithHiddenPhoneNumbers(tx: DBReadTransaction) throws -> [Aci] {
        let sql = """
        ...
        """
        do {
            let serviceIdStrings = try String?.fetchAll(tx.database, sql: sql)
